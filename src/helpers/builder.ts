...
export let builderFun = (
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            let apkFileName = convertAppNameToUserName(
                `${apkName}V${versionName}`
            );
            ...
            if (res1.status == 'success') {
                /* uploading aab*/
                await uploadAbbToApiFun(
                    orderId,
                    res1.data.outputAbb,
                    res1.data.abbNewName
                );
            }

            // await buildApkFun(orderId,ownerId,userName, newVersionCode)
            let res2 = await buildApkUsingSpawnFun(
                orderId,
                apkFileName,
                versionName
            );
            ...
            if (res2.status == 'success') {
                /* uploading apk */
                await uploadApkToApiFun(
                    orderId,
                    res2.data.outputApk,
                    res2.data.apkNewName
                );
            }

            // deleting temp folder
            await rm(`${appTempDir}/${orderId}`, {
                recursive: true,
                force: true,
            });
